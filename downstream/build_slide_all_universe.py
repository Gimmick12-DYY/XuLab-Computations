#!/usr/bin/env python3
"""Build slide table: all_bound positives + all_candidates negatives.

Panel TFs (same 7 as slide_top12k_q05, ZNF768 dropped). Writes:
  downstream/slide_tables/slide_all_universe.{txt,csv}
"""
...
def gather(tf: str) -> dict:
    ...
    out = {
        ...
        "n_candidate_neg": bins.get("n_candidate_neg"),
        "pipes": {},
    }
...
def main() -> int:
    ...
    if missing:
        ...
        for g in rows:
            print(
                f"  {g['tf']}: n_pos={g['n_pos']} n_neg={g['n_neg']} "
                f"auroc_u={g['pipes']['unified']['auroc']} "
                f"auroc_r={g['pipes']['raw']['auroc']}"
            )
        return 1
    ...
    txt = SLIDE / "slide_all_universe.txt"
    csvp = SLIDE / "slide_all_universe.csv"
    txt.write_text("\n".join(lines) + "\n")
