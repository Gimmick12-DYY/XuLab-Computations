...
# cisTopic pipeline (02_build_cistopic_obj.py through 05_impute.py) can consume
# unchanged.
#
...
def _parse_output_mode(spec: str | None, fallback: str = "binary") -> tuple[str, int]:
...
def main() -> int:
    ...
    mode, mode_scale = _parse_output_mode(casc.get("output_mode"), fallback="binary")
    ...
    if mode in ("pseudocount", "float") and looks_binary:
        log.warning(
            ...
            "re-run scBasset with impute.binarize_output: false.", mode, mode)
        mode = "binary"
    ...
    # Tiny manifest so downstream debugging knows where the matrix came from.
    ...
    log.info("Done. mm/ ready for cisTopic 02_build_cistopic_obj.py")
    return 0
