...
# 07_eval_heldout.py
#
# Evaluate scOpen-imputed P(r|c) (read from imputed_Prc_hdf5_all.h5) against
...
#         Score baselines + scOpen on those pairs and report AUROC / AUPRC.
#         This is a goodness-of-fit diagnostic on the training data — not a
#         strict generalization test, since scOpen was fit on these entries.
#
...
#
# Restriction: heldout sampling is by default restricted to scOpen-modeled
# rows (i.e. positives that scOpen could in principle predict; for scOpen this
# is the entire post-filter universe). Pass --sample-space full to ignore that
# restriction and force unmodeled rows to predicted-zero.
# -----------------------------------------------------------------------------
...
def _sample_pairs(
    ...
    the pool — used to keep heldout pairs inside the modeled subset."""
    R, C = mat.shape
...
def _hdf5_scores(
    ...
    outside the modeled set get score 0.0."""
...
def main() -> int:
    ...
    ap.add_argument('--sample-space', type=str, default='modeled',
                    ...
                    help='modeled: restrict heldout sampling to modeled rows '
        ...
        if args.sample_space == 'modeled':
            if not h5_path.exists():
                log.error('Need imputed HDF5 (%s) to determine modeled rows. '
                          'Run 05_impute.py first or pass --sample-space full.',
                          h5_path)
                return 1
            with h5py.File(h5_path, 'r') as h5:
            ...
            log.info('Sample space restricted to %d modeled rows '
                     '(of %d HDF5 rows; %d in mm matrix).',
                     row_pool.size, int(modeled_h5.sum()), len(regions))
            if row_pool.size == 0:
                log.error('No modeled rows are present in the mm matrix; '
    ...
    log.info('Scoring %d pairs against scOpen HDF5 ...', len(rows))
    model_scores = _hdf5_scores(h5_path, rows, cols, full_to_h5_row, h5_col_idx)

    scorers = {'scOpen (HDF5)': model_scores, **_baseline_scores(mat_bin, rows, cols, rng)}
    ordered = ['random', 'region_freq', 'cell_freq', 'independence', 'scOpen (HDF5)']
    results = {name: _metrics(y_true, scorers[name]) for name in ordered}
    for name, m in results.items():
        log.info('  %-22s  AUROC=%.4f  AUPRC=%.4f', name, m['auroc'], m['auprc'])

    title = (f'scOpen held-out reconstruction  (n_pos={len(pos_rows)}, '
