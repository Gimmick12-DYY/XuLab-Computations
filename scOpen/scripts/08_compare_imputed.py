...
#
# Side-by-side comparison of the imputed accessibility P(r|c) against the
# *original* observed matrix (the one written by 01_export_rds_to_mm.R). Reads
# the imputed_Prc_hdf5_all.h5 schema that 05_impute.py writes, so the same
# script works on output from any of the three pipelines (cisTopic, FITS,
# scOpen).
# No UMAP; runs from matrix + HDF5 only.
#
# Two reporting spaces:
#
#   1. filtered space: regions that the model actually predicted (post-filter
#      universe for cisTopic / scOpen, FITS subset for FITS). This is the
#      matrix the imputer saw.
#
...
def main() -> int:
    ...
    fig.suptitle(
        f"Imputation vs original ({args.compare_space}, {R_f}x{C_f} filtered, "
