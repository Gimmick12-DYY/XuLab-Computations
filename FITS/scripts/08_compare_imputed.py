...
# the same imputed_Prc_hdf5_all.h5 schema that 05_impute.py writes for FITS,
# so the same script works for both pipelines (cisTopic and FITS).
# No UMAP; runs from matrix + HDF5 only.
#
# Two reporting spaces:
#
#   1. filtered space: regions that the model actually predicted (FITS subset
#      or cisTopic post-filter universe). This is the matrix the imputer saw.
#
